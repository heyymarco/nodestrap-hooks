...
export const usesGeneralProps = (cssProps: Refs<{}>): PropList => {
    ...
    for (const [propName, propValue] of Object.entries(cssProps)) {
         ...
         * subOpacity
         */
        if ((/^(icon|img|media|arrow(Top|Right|Bottom|Left)?|items|item|sub|logo|toggler|menus|menu|label|control|btn|navBtn|prevBtn|nextBtn|nav|switch|link|bullet|ghost|overlay|card|caption|header|footer|body|tab|breadcrumb|numbered|element|track|tracklower|trackupper|thumb)($|[A-Z])/).test(propName)) continue; // exclude
         ...
         */
        if ((/^(backgGrad|backgOverlay(Img|Size)?|orientation|align|horzAlign|vertAlign|spacing|img|size|valid|invalid|transDuration|(top|bottom|left|right)Transform|fontFamily\w+|fontSize[0-9]+)$/).test(propName)) continue; // exclude
